    ...
    \begin{tikzpicture}[remember picture, overlay]
        ...
        \node [anchor=west, xshift=0.8cm] at (current page.west |- logos) {
            \begin{minipage}{0.85\paperwidth}\raggedright
                ...
                {\usebeamerfont{author}\usebeamercolor[white]{}\scriptsize With thanks to numerous unindicted
                co-conspirators, including \"{O}zg\"{u}r Akg\"{u}n, Blair Archibald,\\Bart Bogaerts,
                Jan Elffers, Stephan Gocht, Ruth Hoffmann, Sonja Kraiczy, Ross McBride,\\Matthew McIlree,
                Samba Ndiaye, Jakob Nordstr\"{o}m, Craig Reilly, Kyle Simpson,\\Christine Solnon,
                James Trimble, William Pettersson, and Patrick Prosser\\}
            \end{minipage}
        };
...
\section{Stealing from CP to Improve Subgraph Algorithms}
...
\begin{frame}{Why Isn't Anyone Else Doing This?}
    \begin{itemize}
        ...
        \item Implementation isn't easy.
            \begin{itemize}
                \item No clean separation between propagation, heuristics, and search.
                \item Efficient nogood propagation is an engineering challenge.
            \end{itemize}
    \end{itemize}
\end{frame}
...
\begin{frame}{Which Optimisation Problems are Hard?}
    \begin{center}\includegraphics[keepaspectratio=true,scale=0.65]{prosser.png}\end{center}
...
\begin{frame}{Optimisation, Refined}
    \begin{center}
    \includegraphics<1>{gen-graph-optimisation.pdf}%
    \includegraphics<2>{gen-graph-optimisation-1.pdf}%
    \includegraphics<3>{gen-graph-optimisation-2.pdf}
        \end{center}
\end{frame}
...
\begin{frame}{Do Labels Make Things Easier or Harder?}
    ...
    \begin{tikzpicture}[scale=0.5,every node/.style={font=\footnotesize}]
        \begin{scope}
            ...
            \node[draw, circle, fill=uofglawn, inner sep=0.5pt] (N10) at (54:2.4) {\phantom{0}};
        ...
        \begin{scope}[xshift=9cm]
            ...
            \node[draw, circle, fill=uofgthistle, inner sep=0.5pt] (N10) at (54:2.4) {\phantom{0}};
...
\begin{frame}{Unfortunately We Can't just Steal SAT Proof Logging}
...
\begin{frame}{The Remarkable Claim}
    \begin{itemize}
        ...
        \item Have the solver output \emph{justifications} in a simpler format.
            \pause
        \item Just using print statements and simple text templates.
        \item Without the solver needing to understand or manipulate proof terms.
    \end{itemize}
\end{frame}

\begin{frame}{Proof Logging for Every Solver}
    \vspace*{-1.0em}
    \begin{center}
        \begin{tikzpicture}
...
\begin{frame}[t]{This Actually Works}
    \only<1>{
        ...
        \begin{itemize}
            ...
            \item All this is efficient:
                \begin{itemize}
                    \item Proof length is proportional to solver runtime.
                    \item Little to no additional computation required.
                \end{itemize}
        \end{itemize}
    }\only<2>{
        \begin{center}
        ...
        \bigskip
        \begin{itemize}
            \item Doesn't know what a graph or an integer variable is.
        \end{itemize}
    }
\end{frame}
