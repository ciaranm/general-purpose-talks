...
\newcommand{\cakepb}{\proofsystemformat{CakePB}\xspace}
...
\author[Ciaran McCreesh et al.]{Markus Anders, Bart Bogaerts, Benjamin Bog\o, Arthur Gontier, Wietze Koops, \textbf{Ciaran
McCreesh}, Matthew McIlree, Magnus Myreen,\\[-0.2cm]Jakob Nordstr\"om, Andy Oertel, Adrian Rebola-Pardo and Yong Kiam Tan}

\title{VeriPB and CakePB 3.0: Formally Verified Pseudo-Boolean Proof Logging for General-Purpose
Combinatorial Solving and Automated Reasoning Algorithms}
...
\begin{frame}{Demotivation}
    ...
    Six years later: trying to use computational experiments to understand the
...
\begin{frame}{The Slide That Keeps Getting Me Into Trouble}
    2021 \toolname{MiniZinc} challenge for constraint programming: for 1.28\% of instances, wrong solutions were claimed.
    \\
    \begin{itemize}
    ...
    SAT solvers don't have this issue. What's their secret?
\end{frame}

\begin{frame}{Proof Logging, Traditionally}
    \vspace*{-1.0em}
    \begin{center}
        \begin{tikzpicture}
...
\begin{frame}{The Problem}
    \begin{itemize}
        \item Lots of different solvers, lots of different reasoning techniques.
        \item For constraint programming alone, hundreds of global constraints.
        \item We need the proof format to be simple enough to trust, but also to be expressive and
            efficient.
    \end{itemize}
\end{frame}

\begin{frame}{The Ludicrous Hypothesis: One Proof Checker, Many Problems}
    \begin{itemize}
        \item A proof system based upon pseudo-Boolean reasoning can do this, without needing to
            know what graphs, integer variables, regular expressions, symmetries, or matchings are.
    \end{itemize}
\end{frame}

\begin{frame}{The First Really Important Idea}
    ...
    \begin{center}
        \begin{tikzpicture}
            ...
            xsep=0.25em, draw, rounded corners=3pt, minimum height=12em, text width=5em, text centered] {
                \only<1>{Checker}\only<2->{\veripb: Elaborator, Justifier, and Trimmer} };
            \node (verifiedchecker) [ right=1.8cm of elaborator.north east, anchor=north west, inner
                xsep=0.25em, draw, rounded corners=3pt, minimum height=8em, text width=4.5em, text
                centered, yshift=-2cm, visible on=<3->] { \cakepb: Verified Checker };
...
\begin{frame}{The Second Really Important Idea}
    \begin{itemize}
        \item Interleave constraint derivations and search (or clause learning), but don't
            differentiate between the two in the proof system.
    \end{itemize}
\end{frame}

\section{Worked Example: Maximum Clique}
...
\begin{onlyenv}<1>%
    I don't want to write a checker for an ad-hoc proof format, though, so let's try using \veripb.

    \begin{center}
        \qrcode[height=3.8cm]{https://veripb.org/}
...
\begin{onlyenv}<2>%
    \begin{itemize}
        \item We have a set of variables $x_i$ that must be given the value $0$ (``false'') or $1$ (``true'').
...
\tikzmark{opblab1a}@noedge1_3\tikzmark{opblab1b} \tikzmark{opbcon1a}-1 x3 -1 x1 >= -1 ;\tikzmark{opbcon1b}
@noedge1_4 -1 x4 -1 x1 >= -1 ;
@noedge1_6 -1 x6 -1 x1 >= -1 ;
@noedge1_7 -1 x7 -1 x1 >= -1 ;
@noedge1_9 -1 x9 -1 x1 >= -1 ;
@noedge2_3 -1 x3 -1 x2 >= -1 ;
@noedge2_4 -1 x4 -1 x2 >= -1 ;
@noedge2_6 -1 x6 -1 x2 >= -1 ;
@noedge2_7 -1 x7 -1 x2 >= -1 ;
@noedge2_9 -1 x9 -1 x2 >= -1 ;
@noedge2_11 -1 x11 -1 x2 >= -1 ;
\textcolor{uofgcobalt}{* \ldots{}and a further 30 similar lines for the remaining non-edges}
\end{Verbatim}
\only<3>{\begin{tikzpicture}[overlay,remember picture]
\end{tikzpicture}}%
\only<4>{\begin{tikzpicture}[overlay,remember picture]
    \node [anchor=north east, text depth=3.8cm, rounded corners=3pt, fill=uofgsunshine, draw,
    yshift=-3.4cm, xshift=-4.3cm] at (current page.north east) {
        \begin{minipage}[t]{0.28\paperwidth}\raggedright
            Has to be minimisation.\\\medskip Multiplication and addition are implicit, so read this as \[
...
\only<5>{\begin{tikzpicture}[overlay,remember picture]
    \node [anchor=north east, text depth=2.9cm, rounded corners=3pt, fill=uofgsunshine, draw,
    yshift=-3.6cm, xshift=-4.3cm] at (current page.north east) {
        \begin{minipage}[t]{0.28\paperwidth}\raggedright
...
\only<6>{\begin{tikzpicture}[overlay,remember picture]
    \node [anchor=north east, text depth=1.0cm, rounded corners=3pt, fill=uofgsunshine, draw, yshift=-3.6cm, xshift=-4.3cm] at (current page.north east) {
        \begin{minipage}[t]{0.28\paperwidth}\raggedright
...
\begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
\tikzmark{pbcliqueheader1}pseudo-Boolean proof version 3.0\tikzmark{pbcliqueheader2}
...
Error: Checking error at example-1.pbp:6

Caused by:
        The constraint is not implied by reverse unit propagation (RUP) from core
        and derived database. Rerun with the option '--trace-failed' to see the
        propagations. A wrong cutting planes ('pol') derivation earlier in the proof
        can also cause this; add '--trace-pol' (with a trace option such as
        '--trace-lines') to trace pol derivations.
\end{Verbatim}
\end{onlyenv}\begin{onlyenv}<3>\vspace*{-0.2cm}%
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize]
$ veripb --trace example.opb example-1.pbp
Running VeriPB version 3.0.2
  Objective: min 1 ~x1 1 ~x2 1 ~x3 1 ~x4 1 ~x5 1 ~x6 1 ~x7 1 ~x8 1 ~x9 1 ~x10 1 ~x11 1 ~x12 0
  ConstraintId 1: 1 ~x1 1 ~x3 >= 1
  ...
  ConstraintId 41: 1 ~x11 1 ~x12 >= 1
line    1: pseudo-Boolean proof version 3.0
line    2: @obj soli x12 x7 x9 ;
  ConstraintId 42: 1 x1 1 x2 1 x3 1 x4 1 x5 1 x6 1 x7 1 x8 1 x9 1 x10 1 x11 1 x12 >= 4
line    3: rup 1 ~x12 1 ~x7 1 ~x9 >= 1 ;
  ConstraintId 43: 1 ~x7 1 ~x9 1 ~x12 >= 1
line    4: rup 1 ~x12 1 ~x7 >= 1 ;
  ConstraintId 44: 1 ~x7 1 ~x12 >= 1
line    5: rup 1 ~x12 >= 1 ;
  ConstraintId 45: 1 ~x12 >= 1
line    6: rup 1 ~x11 1 ~x10 >= 1 ;
Error: Checking error at example-1.pbp:6
\end{Verbatim}
\end{onlyenv}\begin{onlyenv}<4>%
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize]
$ veripb --trace-failed example.opb example-1.pbp
Running VeriPB version 3.0.2
...
\end{onlyenv}
\begin{tikzpicture}[overlay,remember picture]
    \node <2-> [anchor=south east, rounded corners=3pt, yshift=0.4cm, xshift=-0.5cm] at (current page.south east) {
        \begin{tikzpicture}
            \begin{scope}[every node/.style = {font=\scriptsize}]
                \myc=1 \advance\myc by -1 \multiply\myc by -360 \divide\myc by 12 \advance\myc by 60
                \node[anchor=center] (L1) at (\the\myc:1.7) {1};
...
s UNDER ASSERTIONS BOUNDS 8 <= obj <= 8
\end{Verbatim}
...
\end{Verbatim}
\begin{tikzpicture}[overlay,remember picture]
    \node [anchor=north east, text depth=3.2cm, rounded corners=3pt, fill=uofgsunshine, draw, yshift=-1.5cm, xshift=-0.2cm]
    at (current page.north east) {
        \begin{minipage}[t]{0.4\paperwidth}\raggedright
            ...
            verifier doesn't actually need help for two of them.\\ \medskip
...
\end{onlyenv}\begin{onlyenv}<8>
\begin{Verbatim}
$ veripb example.opb example-4.pbp
Running VeriPB version 3.0.2
...
\begin{frame}{Why a General-Purpose Proof System?}
    ...
    \begin{itemize}
        ...
        \item And then we also want to do MaxSAT, constraint programming, \ldots
    \end{itemize}
\end{frame}

\begin{frame}[fragile]{What We Can Do}
\begin{onlyenv}<1>
    \begin{itemize}
        \item Decision, optimisation, enumeration, and (some kinds of counting).
        \item SAT with advanced techniques including parity reasoning and symmetry breaking.
        \item MaxSAT preprocessing and solving using core-guided search, implicit hitting set,
            solution-improving search,
            iterative partial search.
        \item Pseudo-Boolean solving.
        \item Graph problems such as subgraph isomorphism,
            maximum clique, clique enumeration, and maximum common (connected) subgraph, colouring.
        \item MIP preprocessing.
        \item Dynamic programming and decision diagrams.
        \item Classical planning.
        \item Constraint programming for a full set of global constraints, including
            all-different, smart extensional, Hamiltonian circuit, scheduling, multiplication,
            \ldots, fully verified from a \alertblue{human-readable problem description} to a
            \alertblue{human-readable solution}.
    \end{itemize}
\end{onlyenv}\begin{onlyenv}<2>
\begin{Verbatim}
  ( (version 1)
    (variables (x 0 10) (y 0 10) (z 0 10) (p 0 100))
    (constraints
        (c1 lin_equals (2 x 1 y) 12)
        (c2 multiply x y p)
        (c3 all_different (x y z))
        (c4 divide y x z))
    (prob_type (maximize p))
  )

  $ ./solve_and_verify.sh demo
  p=16 x=2 y=8 z=4
  p=18 x=3 y=6 z=2
