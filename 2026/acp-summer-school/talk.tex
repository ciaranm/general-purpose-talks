...
\begin{frame}{Pseudo-Boolean Notation Recap}
    ?? literals, less than / greater than, etc. Logic and the connection to CNF.
\end{frame}
...
\begin{frame}{Pseudo-Boolean Reification Recap}
    ...
    \end{equation*}
    for an $M$ big enough that the constraint says nothing at all when $b$ is false.

    \bigskip

    \toolname{VeriPB} writes it for you: \texttt{b ==> \ldots} and \texttt{b <== \ldots} normalise to
    precisely that constraint.
\end{frame}
...
\begin{frame}{The Direct Encoding}
    ...
    Upper case $X$ for a constraint programming variable, lower case for pseudo-Boolean literals.
