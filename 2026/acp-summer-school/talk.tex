...
rup >= 1 ;
    ...
    \begin{minipage}[t]{0.60\framewidth}
        \begin{itemize}
            \item<2->
                Ten guesses became two, and eleven lines became three.
            \item<3->
                And the three lines are \alertred{\texttt{REJECTED}}. Bounding $X_2$, $X_3$, $X_4$
...
\begin{frame}[fragile]{Debugging a Failed Proof}
    ...
    \begin{semiverbatim}\footnotesize
Error: Checking error at search.pbp:10, not implied by RUP
\uncover<2->{
Propagation check failed! The propagation had the following trail:
    ...
    \begin{itemize}
        \item<3->
            Line $10$ is where we \alertblue{claimed} something, which is rarely where the problem
            is: everything above it is what should have made the claim follow.
        \item<4->
...
\begin{frame}[fragile]{Assertions for Propagations}
    The cheat: \texttt{a} adds a constraint without checking it. Same proof, with the two
    all-different failures asserted rather than derived:

    \begin{semiverbatim}\footnotesize
% decide x1 = 0
rup x1eq0 ==> 1 ~x2ge2 >= 1 ;     \textrm{\emph{and the same for x3 and x4}}
a   x1eq0 ==> >= 1 : : all_different_hall ;
rup x1eq0 ==> >= 1 ;
% decide x1 != 0                  \textrm{\emph{\ldots and symmetrically over here}}
rup >= 1 ;
    \end{semiverbatim}
