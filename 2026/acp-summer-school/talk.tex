...
\begin{frame}[fragile]{Assertions for Propagations}
    The cheat: \texttt{a} adds a constraint without checking it. Same example, with the two Hall
    violations asserted rather than derived:

    \begin{semiverbatim}\footnotesize
% decide W = 0
rup x1eq0 ==> 1 ~x2ge2 >= 1 ;      \textrm{\emph{and the same for Y and Z}}
a   x1eq0 ==> >= 1 : : all_different_hall ;
rup x1eq0 ==> >= 1 ;
% decide W != 0                    \textrm{\emph{\ldots and symmetrically over here}}
rup >= 1 ;
    \end{semiverbatim}
