...
\begin{frame}[fragile]{Backtracking Search}
    $X, Y, Z \in \{0, 1\}$, all different, and no propagation yet: a clash is noticed only once two
    of them have values.
    ...
    \begin{minipage}[t]{0.42\framewidth}
        ...
        \begin{center}
            \begin{tikzpicture}[
                level 1/.style={level distance=0.75cm, sibling distance=2.6cm},
                level 2/.style={level distance=0.75cm, sibling distance=1.3cm},
                level 3/.style={level distance=0.75cm, sibling distance=0.7cm}]
                \node [treenode] {$X$}
...
\uncover<2->{rup x1eq0 x2eq0 ==> >= 1 ;}
\uncover<3->{rup x1eq0 ~x2eq0 x3eq0 ==> >= 1 ;}
\uncover<4->{rup x1eq0 ~x2eq0 ~x3eq0 ==> >= 1 ;}
...
\uncover<8->{rup >= 1 ;}
    ...
    \onslide<9->{A guess needs no justification. Only a \deadend{} does, and it is one line: those
    decisions cannot all hold. So the proof is the tree, rotated a quarter turn, and the last dead
    end is the root.}
\end{frame}

\begin{frame}[fragile]{Propagations Aren't Necessarily RUP}
    \begin{minipage}[t]{0.44\framewidth}
        $W \in \{0,1\}$ and $X, Y, Z \in \{0,1,2\}$, with
        \begin{align*}
            &\operatorname{all\_different}(X, Y, Z) \\
            &W + X + Y + Z \le 2 \\
            &X + Y + Z - W \le 1
        \end{align*}
        ...
        Neither inequality prunes anything at the root, and three variables over three values are
        not a violated Hall set, so we have to guess.
...
rup >= 1 ;
        ...
        \begin{itemize}
            \item<2->
                \alertred{\texttt{REJECTED}}: the checker cannot reach those contradictions.
            \item<3->
                Each branch bounds $X$, $Y$, $Z$ by $1$, which unit propagation \alertblue{can}
                redo, since deciding $W$ forces a bit of each.
            \item<4->
                That leaves three variables in two values, which it \alertred{cannot}: nothing is
                unit, and counting is a cutting planes argument.
        \end{itemize}
    \end{minipage}
\end{frame}
