...
\begin{frame}{A Slightly Random Value-Ordering Heuristic}
    ...
    \[ p(v') = \frac{2^{\deg(v')}}{\sum_{w \in D_v}{2^{\deg(w)}}} \]
        ...
        \item Final ingredient: frequently \textcolor{uofgcobalt}{restart} the search, remembering
            not to repeat anything already done using \textcolor{uofgcobalt}{nogoods}.
    \end{itemize}
\end{frame}
...
\begin{frame}{Fun Challenges}
    \begin{itemize}
        \item Symmetries (particularly with counting or proof logging).
        \item Knowing which filtering rules to use, when.
        \item Large, labelled target graphs.
