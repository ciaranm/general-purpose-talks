...
\begin{frame}{Maximum Common Subgraph}
    \only<1>{\centering\begin{tikzpicture}
        \begin{scope}
        ...
        \begin{scope}[xshift=6cm]
            ...
            \draw [very thick] (M8) -- (M9);
        \end{scope}
    \end{tikzpicture}}\only<2>{\centering\begin{tikzpicture}[scale=0.33]%{{{
        \begin{scope}
...
\begin{frame}{Increasing Sortedness Decreases Search Space Size}
    \centering\includegraphics{graph-reorder-cumulative-nodes.pdf}
\end{frame}
...
\begin{frame}{Phase Transitions}
    \centering\includegraphics{graph-phase-transition.pdf}
\end{frame}
...
\begin{frame}{Special Award-Winning Crime That Gets a Slide of its Own}
    \begin{itemize}
        \item Take one of my solvers.
        \item Modify the timing code to report CPU time instead of wallclock time.
        \item Make the code five times longer with features that literally do nothing.
        \item Claim it's better by benchmarking against the original solver with its original timing
            code.
        \item Publish in top AI venue by resubmitting over and over again until you win the reviewer
            competence lottery.
    \end{itemize}
\end{frame}
...
\begin{frame}{A Summary of Crimes Witnessed}
    \begin{itemize}
        ...
        \item Multiplying other people's runtimes by mysterious scaling factors rather than
