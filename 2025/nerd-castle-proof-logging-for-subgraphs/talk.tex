...
\end{Verbatim}
\end{minipage}\begin{minipage}{0.45\textwidth}\begin{onlyenv}<16->
\begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
...
min: 1 ~x1 1 ~x2 1 ~x3 1 ~x4 1 ~x5 1 ~x6 1 ~x7 1 ~x8 1 ~x9 1 ~x10 1 ~x11 1 ~x12 ;
...
\begin{frame}[fragile]{Colour Classes are At-Most-One Constraints}
    \begin{onlyenv}<1>
...
from which it is much easier to see that taking both vertices 10 and 11 isn't going to work.
    ...
    \begin{onlyenv}<2>
        \begin{itemize}
            \item Don't really want to put all this in the OPB file:
                \begin{itemize}
                    \item We'll end up using exponentially many colour classes sometimes.
                    \item How do we know the colour classes are still correct?
                \end{itemize}
            \item Could we add the at-most-one constraints during the proof?
        \end{itemize}
    \end{onlyenv}%
\begin{onlyenv}<3>
\begin{Verbatim}[commandchars=\\\{\},codes={\catcode`$=3\catcode`^=7}]
pseudo-Boolean proof version 2.0
@objective soli x7 x9 x12
rup 1 ~x12 1 ~x7 >= 1 ;
rup 1 ~x12 >= 1 ;
a 1 x2 1 x4 1 x5 1 x6 1 x7 1 x8 1 x10 1 x11 1 x12 >= 3 ;
rup 1 ~x11 1 ~x10 >= 1 ;
rup 1 ~x11 >= 1 ;
@objective soli x1 x2 x5 x8
rup 1 ~x8 1 ~x5 >= 1 ;
rup 1 ~x8 >= 1 ;
a 1 x8 1 x11 1 x12 >= 2 ;
rup >= 1 ;
output NONE
conclusion BOUNDS 8 8
...
\hphantom{2} \overline{x}_6 +
\hphantom{2} \overline{x}_9
          \ge \only<2>{\frac{3}{2}\phantom{2}}\only<3->{2\phantom{\frac{3}{2}}}\phantom{\frac{3}{2}} \\
          && \uncover<4->{\text{i.e.}~ & x_1 + x_6 + x_9 \le 1}
    \end{align*}}
  \uncover<5->{
    This generalises
...
u 1 ~x12 1 ~x7 1 ~x9 >= 1 ;
u 1 ~x12 1 ~x7 >= 1 ;
p @noedge1_6 @noedge1_9 + @noedge6_9 + 2 d @obj +
u 1 ~x12 >= 1 ;
p @noedge1_3 @noedge1_9 + @noedge3_9 + 2 d @obj +
u 1 ~x11 1 ~x10 >= 1 ;
p @noedge1_3 @noedge1_7 + @noedge3_7 + 2 d @obj +
u 1 ~x11 >= 1 ;
@obj soli x8 x5 x2 x1
u 1 ~x8 1 ~x5 >= 1 ;
p @obj @noedge1_9 +
u 1 ~x8 >= 1 ;
p @noedge1_3 @noedge1_7 + @noedge3_7 + 2 d @noedge2_4 @noedge2_9 + @noedge4_9 + 2 d +
...
\begin{frame}{Why a General-Purpose Proof System?}
    ...
    \begin{itemize}
        ...
        \item Even more when we look at problems like maximum weight clique.
            ...
            between multiple colours), but it turns out they can all be handled by adding in an extra
            multiplication in the proof step.
...
$ veripb brock200_4.opb brock200_4.pbp
Verification succeeded.

$ grep conclusion brock200_4.pbp
conclusion BOUNDS 183 183

$ cake_pb_clique brock200_4.clq > brock200_4.verifiedopb

$ veripb brock200_4.verifiedopb brock200_4.pbp --proofOutput brock200_4.corepb
Verification succeeded.
