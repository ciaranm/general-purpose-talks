...
at (current page.center) {
        \includegraphics[keepaspectratio=true,scale=0.24]{wrong.png}
};
...
\begin{Verbatim}
$ pboxide_veripb example.opb example-1.pbp
Running PBOxide VeriPB version 0.2.0-357263b
Error: Verification error at example-1.pbp:6!

Caused by:
        The constraint is not implied by reverse unit propagation (RUP).
\end{Verbatim}
\end{onlyenv}\begin{onlyenv}<3>%
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize]
$ pboxide_veripb --trace example.opb example-1.pbp
Running PBOxide VeriPB version 0.2.0-357263b
...
\begin{Verbatim}[commandchars=\\\{\},fontsize=\footnotesize]
$ pboxide_veripb --trace-failed example.opb example-1.pbp
  ...
line    6: rup 1 ~x11 1 ~x10 >= 1 ;
Propagation check failed! The propagation had the following trail:
  propagatons in format: <assignment> (<reason constraint>)
    ~x12 (1 ~x12 >= 1)
    x10 (1 x10 1 x11 >= 2)
    x11 (1 x10 1 x11 >= 2)
    ~x6 (1 ~x6 1 ~x10 >= 1)
...
\end{onlyenv}
\begin{tikzpicture}[overlay,remember picture]
    \node <2-> [anchor=south east, rounded corners=3pt, yshift=0.2cm, xshift=-0.5cm] at (current page.south east) {
        \begin{tikzpicture}
            \begin{scope}[every node/.style = {font=\scriptsize}]
                \myc=1 \advance\myc by -1 \multiply\myc by -360 \divide\myc by 12 \advance\myc by 60
                \node[anchor=center] (L1) at (\the\myc:1.7) {1};
...
\begin{Verbatim}
$ pboxide_veripb example.opb example-2.pbp 
Running PBOxide VeriPB version 0.2.0-357263b
s VERIFIED BOUNDS 8 <= obj <= 8
Warning: The proof used unchecked assumptions.
\end{Verbatim}
...
\begin{Verbatim}
$ pboxide_veripb example.opb example-3.pbp
Running PBOxide VeriPB version 0.2.0-357263b
s VERIFIED BOUNDS 8 <= obj <= 8
Warning: The proof used unchecked assumptions.
\end{Verbatim}
...
\end{onlyenv}\begin{onlyenv}<8>
\begin{Verbatim}
$ pboxide_veripb example.opb example-4.pbp
Running PBOxide VeriPB version 0.2.0-357263b
s VERIFIED BOUNDS 8 <= obj <= 8
\end{Verbatim}
\end{onlyenv}
...
\smallskip
\begin{Verbatim}[fontsize=\scriptsize]
$ time pboxide_veripb --elaborate brock200_4.corepb brock200_4.verifiedopb brock200_4.pbp
...
\end{tikzpicture}
\begin{tikzpicture}[overlay,remember picture]
    \node <3> [anchor=south east, yshift=0cm] at (current page.south east) {
        \begin{minipage}[t]{0.25\paperwidth}\raggedright
        \tiny ChatGPT, can you draw me a picture of a \\
             sophisticated pigeon? \\
        \includegraphics[scale=0.1]{sophisticatedpigeon.png}
        \end{minipage}
    };
    \node <4> [anchor=south east, yshift=0cm] at (current page.south east) {
        \begin{minipage}[t]{0.25\paperwidth}\raggedright
        \tiny I think a sophisticated pigeon should have a monocle and a twirly mustache, rather
            than glasses and a beard. \\
        \includegraphics[scale=0.1]{monoclepigeon.png}
        \end{minipage}
    };
\end{tikzpicture}%
\end{frame}
...
pol @adj\_p\_t\_q @adj\_p\_t\_r + @adj\_p\_t\_s +    \textcolor{uofgcobalt}{% sum adjacency constraints}
    @inj\_u + @inj\_v + ;                     \textcolor{uofgcobalt}{% sum injectivity constraints}
ia 1 ~xp\_t >= 1 : \textcolor{uofgpillarbox}{-1} ;                      \textcolor{uofgcobalt}{% desired conclusion is syntactically implied}
\end{Verbatim}
\end{frame}
...
\begin{frame}{Reformulation}
    ...
    \begin{itemize}
        \item We can encode this reduction using cutting planes rules.
        \item No need to modify the clique solver, either.
    \end{itemize}
\end{frame}
...
\begin{frame}{Pattern Graph Symmetries}
\begin{minipage}{0.6\paperwidth}
    \begin{itemize}
        \item If a solution exists, a solution where $C < D$ exists.
        \item Might want to decide the constraint $C < D$ dynamically during search, or even to
            change constraint to $F < E$ for different subproblems.
        \item Enumeration proofs: ``unsatisfiable, except for all the solutions I listed''. But what
            does it mean to count or enumerate solutions under symmetries?
    \end{itemize}
\end{minipage}\hfill\begin{minipage}{0.3\paperwidth}
    \begin{center}
    \begin{tikzpicture}
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Na) at (1,  0) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Nb) at (1, -1.5) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Nc) at (0, -3) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Nd) at (2, -3) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Ne) at (0, -4.5) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (Nf) at (2, -4.5) {\vphantom{1}};
        \node at (Na) {A};
        \node at (Nb) {B};
        \node at (Nc) {C};
        \node at (Nd) {D};
        ...
        \draw [ultra thick] (Nc) -- (Nd);
        \draw [ultra thick] (Nb) -- (Nd);
        \draw [ultra thick] (Ne) -- (Nf);
        \draw [ultra thick] (Nc) -- (Ne);
        \draw [ultra thick] (Nd) -- (Nf);
    \end{tikzpicture}
    \end{center}
\end{minipage}%
\end{frame}
...
\begin{frame}{Target Graph (Conditional) Symmetries}
    \begin{center}
    \begin{tikzpicture}
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N1) at (0,  0) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N2) at (1,  0) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N3) at (2,  0) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N4) at (3,  0) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N5) at (0,  -1) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N6) at (1,  -1) {\vphantom{1}};
        \node <1> [draw, circle, fill=white, inner sep=4pt, font=\bfseries] (N7) at (2,  -1) {\vphantom{1}};
        \node at (N1) {1};
        \node at (N2) {2};
        \node at (N3) {3};
        \node at (N4) {4};
    ...
    \begin{itemize}
        \item If $4$ removed from all domains dynamically during search (all pattern vertices of
            degree $1$ already assigned elsewhere?), a symmetry appears.
        \item If $3$ and $7$ removed, even more symmetries appear.
        \item We can still count when exploiting target symmetries, but we now seem to need
            non-trivial amounts of group theory to explain correctness efficiently.
    \end{itemize}
\end{frame}
...
\begin{frame}{Dominance}
    ...
    \begin{tikzpicture}
        ...
        \foreach \n in {2}{
            \myc=\n \advance\myc by -1 \multiply\myc by -360 \divide\myc by 12 \advance\myc by 67
            \node[anchor=center] (L\n) at (\the\myc:1.7) {\n};
            \node[anchor=center, circle, fill=uofgcobalt, inner sep=2pt] (N\n) at (\the\myc:1.42) {};
        }
...
\medskip
    ...
    \begin{itemize}\item Every neighbour of 2b is also a neighbour of 2.
            \item Too expensive to detect upfront, so we catch it on backtrack instead.
    \end{itemize}
\end{frame}
