...
\begin{frame}{This Actually Works!}
    ...
    \bigskip
    \begin{itemize}
        \item Produces proofs that interleave backtracking search with additional
            justifications for propagators.
...
\end{minipage}\begin{minipage}{12cm}
    ...
    \begin{itemize}
        \item<2-> $\operatorname{routes}(\truck{uofgrust}, 1) < \operatorname{routes}(\truck{uofgcobalt}, 1)$
        \item<3-> $\operatorname{routes}(\truck{uofgcobalt}, 1) < \operatorname{routes}(\truck{uofgsunshine}, 1)$
        \item<4-> $\operatorname{routes}(\truck{uofgsunshine}, 1) < \operatorname{routes}(\truck{uofgleaf}, 1)$ \only<5->{Maybe?}
        \item<6-> $\operatorname{vehicle}(\drivera) < \operatorname{vehicle}(\driverb)$ etc\ldots
        \item<7-> $\operatorname{routes}(\truck{uofgrust}, 1) < \operatorname{routes}(\truck{uofgrust}, 2)$ \only<8->{Sus.}
    \end{itemize}
\end{minipage}
\end{center}
\end{frame}
...
\begin{frame}{For Example\ldots}
    \[
        \trashpanda \rightarrow \parcel_1 \oplus \parcel_2 \oplus \parcel_3 \oplus \parcel_4
    \]

    \pause
    \begin{itemize}
        \item Want to show: WLOS, $\trashpanda \mapsto \parcel_1$.
            \pause
        \item Show $\trashpanda \not\mapsto \parcel_4$:
            \begin{itemize}
                \item If you give me an assignment that satisfies all the constraints, but puts
                    $\trashpanda \mapsto \parcel_4$
                \item then I give you an assignment that satisfies all the
            constraints and $\trashpanda \not\mapsto \parcel_4$ by $\parcel_3 \leftrightarrows
            \parcel_4$.
            \end{itemize}
            \pause
        \item Show $\trashpanda \not\mapsto \parcel_3$ using $\parcel_2 \leftrightarrows \parcel_3$.
            \pause
        \item Show $\trashpanda \not\mapsto \parcel_2$ using $\parcel_1 \leftrightarrows \parcel_2$.
            \pause
        \item Can now derive $\trashpanda \mapsto \parcel_1$ using resolution.
            \pause
        \item But a bit awkward: can't just go straight to $\trashpanda \mapsto \parcel_1$ because
            we need to know where it is to specify the fixup.
        \item Not clear how to handle e.g.\ symmetries on two generators this way.
    \end{itemize}
\end{frame}
...
\begin{frame}{Maybe This Will Make Things Clearer}
    \[
        \trashpanda \rightarrow \parcel_1 \oplus \parcel_2 \oplus \parcel_3 \oplus \parcel_4
    \]
    ...
    \begin{itemize}
        \item Want to show: WLOS, $\trashpanda \mapsto \parcel_1$.
            \pause
        \item Introduce objective function to minimise $\sum_i 2^i \left(\trashpanda \overset{?}{\mapsto}
            \parcel_i\right)$.
            \pause
        \item If you give me an assignment that satisfies all the constraints, but has
            $\trashpanda \not\mapsto \parcel_1$
        \item Then consider doing $\parcel_4 \rightarrow \parcel_3 \rightarrow \parcel_2 \rightarrow
            \parcel_1 \rightarrow \parcel_4$, which reduces the objective.
            \pause
        \item Now either $\trashpanda \mapsto \parcel_1$, so we are done, or we apply the rotation
            again, and reduce the objective.
            \pause
        \small \item Now either $\trashpanda \mapsto \parcel_1$, so we are done, or we apply the rotation
            again, and reduce the objective.
            \pause
        \footnotesize \item Now either $\trashpanda \mapsto \parcel_1$, so we are done, or we apply the rotation
            again, and reduce the objective.
            \pause
        \scriptsize \item Now either $\trashpanda \mapsto \parcel_1$, so we are done, or we apply the rotation
            again, and reduce the objective.
            \pause
        \normalsize \item Objective function has only finitely many values, so we can't keep doing this forever.
    \end{itemize}
\end{frame}
