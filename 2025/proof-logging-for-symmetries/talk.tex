...
\end{minipage}\begin{minipage}{12cm}
    ...
    \begin{itemize}
        \item<2-> $\operatorname{routes}(\truck{uofgrust}, 0) < \operatorname{routes}(\truck{uofgcobalt}, 0)$
        \item<3-> $\operatorname{routes}(\truck{uofgcobalt}, 0) < \operatorname{routes}(\truck{uofgsunshine}, 0)$
        \item<4-> $\operatorname{routes}(\truck{uofgsunshine}, 0) < \operatorname{routes}(\truck{uofgleaf}, 0)$ \only<5->{Maybe?}
        \item<6-> $\operatorname{vehicle}(\drivera) < \operatorname{vehicle}(\driverb)$ etc\ldots
        \item<7-> $\operatorname{routes}(\truck{uofgrust}, 0) < \operatorname{routes}(\truck{uofgrust}, 1)$ \only<8->{Sus.}
    \end{itemize}
\end{minipage}
\end{center}
\end{frame}
    ...
    \begin{itemize}
        ...
        \item Show $\trashpanda \not\mapsto \parcel_4$:
            \begin{itemize}
                ...
                \item then I give you an assigment that satisfies all the
