...
\begin{frame}{What is a Proof?}
    \includegraphics[scale=0.34]{shortest-math-paper.jpg}
\begin{tikzpicture}[overlay,remember picture]
    \node <2-3> (easytocheck) [anchor=center, font=\large, xshift=5cm, yshift=-0.53cm] at (current page.center) { Easy to check? };
    \draw <2-3> [thick, rounded corners, color=uofglawn] ($(current page.center)+(-4.6cm,-0.30cm)$) rectangle ($(current page.center)+(0.3cm,-0.76cm)$);
    \draw <2-3> [->, thick, color=uofglawn] ($(current page.center)+(0.3cm,-0.53cm)$) -> (easytocheck);
\end{tikzpicture}%
\begin{tikzpicture}[overlay,remember picture]
    \node <3> (notsosure) [anchor=center, font=\large, xshift=5cm, yshift=-1.06cm] at (current page.center) { Not so sure\ldots };
    \draw <3> [thick, rounded corners, color=uofgpillarbox] ($(current page.center)+(-5.8cm,-0.88cm)$) rectangle ($(current page.center)+(-4.6cm,-1.24cm)$);
    \draw <3> [->, thick, color=uofgpillarbox] ($(current page.center)+(-4.6cm,-1.06cm)$) -> (notsosure);
\end{tikzpicture}%
\end{frame}

\begin{frame}[fragile]{Let's Ask ChatGPT!}
    \only<1-3>{
 ...
 27^5 + 84^5 + 110^5 + 133^5 = 144^5
    \]
      ...
      \begin{align*}
          ...
          84^5 &= 418211328 \\
          110^5 &= 1610510000 \\
          133^5 &= \only<1>{433494437}\only<2->{\textcolor{uofgpillarbox}{433494437}} & \uncover<2->{\text{\textcolor{uofgpillarbox}{WRONG!}}} \\
          14348907 + 418211328 + 1610510000 + 433494437 &= 6191730672 & \uncover<3->{\text{\textcolor{uofgpillarbox}{WRONG!}}} \\
          144^5 &= 6191730672
      \end{align*}
      It checks out.
  }\only<4-5>{
...
\begin{frame}{Let's Do This Properly}
     \begin{itemize}
...
\begin{frame}{Unit Propagation for Clauses}
    ...
    Now must set $d$ to true to satisfy second clause.
\end{frame}
